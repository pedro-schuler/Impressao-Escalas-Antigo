...
foreach ($escalasA as $esc => $escN) {
    ...
    foreach ($e as $el) {
        ...
        for ($i = 2; $i < 33; $i++) {
            ...
            if (!$temTurno) {
                if (!in_array("EA", $diaEx) && !in_array("A", $diaEx) && !in_array("--", $diaEx)) {
...
//media carga horaria
$mediaReal = ceil(array_sum($chOpr) / $qtdEfetivoEscala);
...
$html = <<<EOD
    <table cellpadding="2" width="1130">
        ...
        <tr style="font-weight: bold; font-size: 9;">
            ...
            <td style="border-left:0px solid black;border-bottom:0px solid black;border-right:0px solid black;font-size: 8;">$mediaReal/$mediaComTodos</td>
